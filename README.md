...
- 🚀 **想立即体验？** → [小白用户快速指南](#-小白用户快速指南)
- 📦 **要集成到项目？** → [将 WebAssembly 用到您的项目中](#-将-webassembly-用到您的项目中)
- 🔍 **遇到问题？** → [故障排除](#-故障排除)
- 💻 **要修改代码？** → [高级用户：修改和重新构建](#️-高级用户修改和重新构建-可选)
- 📁 **找不到文件？** → [项目完整结构](#-项目完整结构)
...
3. **🌐 打开浏览器**
   ...
   - 🎉 您应该能看到一个可交互的 WebAssembly 演示页面！

4. **🧪 测试功能**
   - 在页面上输入数字，点击"加法"、"乘法"按钮
   - 输入名字，点击"生成问候语"
   - 所有计算都是由 Rust 编写的 WebAssembly 模块完成的！
...
> **⚠️ 重要提示**: 本指令适用于 **Zig 0.15.1+** 版本。旧版本的编译方法已经过时，请使用以下正确的方法。
...
#### 优化等级选择
- `-O Debug` - 调试版本，编译快，体积大
- `-O ReleaseSmall` - 最小体积，适合Web使用 **(推荐)**
- `-O ReleaseFast` - 最快执行速度
- `-O ReleaseSafe` - 安全版本，保留检查

### 📋 参数说明

| 参数 | 说明 |
|------|------|
| `zig build-exe` | 构建可执行文件格式 |
| `-target wasm32-freestanding` | 32位WASM目标，无操作系统 |
| `-fno-entry` | 不需要main入口函数 |
| `--export=函数名` | 导出指定函数供外部调用 |
| `-O ReleaseSmall` | 优化为最小体积 (推荐) |
| `--name 文件名` | 指定输出文件名 (可选) |
